...
$(function () {
    ...
    $('.assign_terminal').click(function(e){
        e.preventDefault();
        $('#assign_terminal_modal').show();
        $('#modal_user_id').val($(this).data('userid'));
    });

    $('#assign_terminal_modal').find('.close').click(function(){
        $('#assign_terminal_modal').hide();
    });

    $('#assign_terminal_datetime').daterangepicker({timePicker: false, timePickerIncrement: 30, format: 'MM/DD/YYYY H:mm'});

    $('#submit_assign_terminal').click(function () {
       $('#assign_terminal_form').submit();
    });

    $("#list_user_reports_table").dataTable();
});